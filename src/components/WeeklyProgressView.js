...
const WeeklyProgressView = ({ navigation }) => {
  ...
  const fetchFirstUseDate = async () => {
    ...
    try {
      ...
      Alert.alert('Error', 'No se pudo cargar la fecha de inicio de uso. Por favor, intenta de nuevo.');
    }
  };
  ...
  const fetchAggregatedProgressData = async () => {
    ...
    try {
      ...
      const aggregatedData = viewMode === 'weekly' ? groupByWeek(data) : groupByMonth(data);
      setProgressList(aggregatedData);
    } catch (error) {
      console.error('Error fetching progress data:', error);
      Alert.alert('Error', 'No se pudo cargar el progreso. Por favor, intenta de nuevo.');
    }
  };
  ...
  const groupByWeek = (data) => {
    ...
    data.forEach(entry => {
      ...
      if (!groupedData[weekNumber]) {
        groupedData[weekNumber] = { sleepHours: 0, exerciseHours: 0, calories: 0, entries: 0 };
      }
    ...
    return Object.keys(groupedData).map(week => ({
      week,
      ...groupedData[week],
      sleepHours: (groupedData[week].sleepHours / groupedData[week].entries).toFixed(1),
      exerciseHours: (groupedData[week].exerciseHours / groupedData[week].entries).toFixed(1),
      calories: (groupedData[week].calories / groupedData[week].entries).toFixed(0),
    }));
  };
  ...
  const groupByMonth = (data) => {
    ...
    data.forEach(entry => {
      ...
      if (!groupedData[month]) {
        groupedData[month] = { sleepHours: 0, exerciseHours: 0, calories: 0, entries: 0 };
      }
    ...
    return Object.keys(groupedData).map(month => ({
      month,
      ...groupedData[month],
      sleepHours: (groupedData[month].sleepHours / groupedData[month].entries).toFixed(1),
      exerciseHours: (groupedData[month].exerciseHours / groupedData[month].entries).toFixed(1),
      calories: (groupedData[month].calories / groupedData[month].entries).toFixed(0),
    }));
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <Text style={styles.title}>Progreso {viewMode === 'weekly' ? 'Semanal' : 'Mensual'}</Text>
      ...
      <FlatList
        data={progressList}
        keyExtractor={(item) => viewMode === 'weekly' ? `week-${item.week}` : `month-${item.month}`}
        renderItem={({ item }) => (
          <View style={styles.progressItem}>
            <Text style={styles.progressText}>
              {viewMode === 'weekly' ? `Semana ${item.week} de uso` : `Mes ${item.month}`}
            </Text>
            <Text style={styles.progressText}>Sueño Promedio: {item.sleepHours} hrs</Text>
            <Text style={styles.progressText}>Ejercicio Promedio: {item.exerciseHours} hrs</Text>
            <Text style={styles.progressText}>Calorías Promedio: {item.calories}</Text>
          </View>
        )}
      />
    </SafeAreaView>
